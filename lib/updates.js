...
exports.update_sale = function(doc,req){
    var sale = require('./types').sale;
    var saleForm = new Form(sale);
    saleForm.validate(req);
    log("update_sale : " + saleForm.values._id);
    if(saleForm.values._id == null || !saleForm.values._rev)
    {
        saleForm.values._id = req.uuid;
        log("New sales record, assigning _id " + saleForm.values._id + " _rev : " + saleForm.values._rev);
    }
    else
    {

        log("Found sale _id : " + saleForm.values._id + " updating." + " _rev : " + saleForm.values._rev);
    }
    if(saleForm.isValid()){
        // add the new expense document by returning the validated values.
        var saleDb = saleForm.values;
        //expdb.ReceiptDate = new Date(expdb.year,expdb.month,expdb.day);
        log("update_sale : Valid update saving...");
        //return[expdb,{content:"Created new expense entry in the amount of : " + expForm.values.total}];
        return [saleDb,utils.redirect(req,  "/income")]

        //http://localhost:5989/simplestpossibleform/_design/business/_rewrite/simplestpossibleform/_design/business/_rewrite/update_expense
    }
    else
    {
        log("update_sale : Invalid values detected, create new.")
        var content = templates.render('form.html',req,
            {
                form_title:"Sales Entry",
                method:'POST',
                action :  "update_sale",
